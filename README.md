...
## Два режима: для читающих и не читающих

При запуске игра спрашивает, кто играет:

- **🐣 По картинкам, с голосом** — для самых маленьких, кто ещё не умеет
  читать. Все диалоги и подсказки показываются крупными пиктограммами
  (🐟 3/5, 🗼🔥, ❤️) и озвучиваются голосом. Жёлтая стрелка-компас всегда
  показывает, куда лететь, а когда собраны все подсказки — над «твоим
  человеком» появляется сердечко. Карточки жителей в дневнике можно
  тапнуть — игра расскажет о жителе вслух.
- **📖 С текстом** — обычный режим с репликами и текстовыми подсказками.
  Стрелка-компас тоже помогает не заблудиться.

![Выбор режима](docs/screenshot-mode-select.png)
![Детский диалог пиктограммами](docs/screenshot-kid-dialog.png)
