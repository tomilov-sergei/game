...
## Понятно и тем, кто ещё не читает

Игра дружелюбна к самым маленьким, при этом остаётся интересной старшим:

- Диалоги показывают **крупные пиктограммы и текст одновременно**
  (🗼🔥 + «Подлети к маяку и зажги его своим огнём!») — старшая сестра
  может прочитать младшей, а картинка понятна и без слов
- Кнопки в диалогах — с иконкой и подписью: 🤝 «Чем помочь?»,
  ❤️ «Ты — мой человек!», 👋 «Улететь»
- Реплики жителей и подсказки **озвучиваются голосом** (Web Speech API);
  выключается вместе со звуком (M / 🔊)
- Трекер заданий — пиктограммы со счётчиками: 🐟 Рыба: 3/5
- Карточки жителей в дневнике можно тапнуть — игра расскажет о жителе вслух
- Когда собраны все 6 подсказок, над «твоим человеком» появляется сердечко —
  финал доступен даже тем, кто не читает
- Никаких навигационных стрелок — остров исследуется свободно,
  ориентиры: миникарта и маркеры «!» над жителями

![Диалог: пиктограммы и текст](docs/screenshot-dialog.png)
...
- **Музыка и звуки** — тихая колыбельная-пентатоника (WebAudio, без файлов),
  шум моря у берега, ветер в полёте, блеяние овец и хрюканье кабанов
- **Сохранение** — прогресс хранится в браузере; после перерыва
  нажми «Продолжить»
- **Посадка** — `C` / `🐾`: дракон садится и ходит по земле; тапни по нему —
  он заурчит и завиляет хвостом, а вверх полетят сердечки
- **Высота полёта** — `R`/`F` (на клавиатуре) — взмыть повыше или прижаться
  к земле
- **10 драконьих чешуек** спрятаны по острову — собери все, и у Искры
  станут золотые рожки
- **Овцы убегают** от дракона во время задания, а **кабаны** могут
  комично броситься в ответ — не летай слишком низко!
- **После финала** — свободная игра: твой человек летает верхом на Искре,
  жители дают маленькие повторяемые просьбы, а счётчик дружбы растёт

![Полёт со своим человеком](docs/screenshot-rider.png)
